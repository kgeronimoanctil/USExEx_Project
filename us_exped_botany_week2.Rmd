...
```{r echo=FALSE}
a1 <- count(btny_unique) %>% 
  rename(count = n) %>% 
  add_column(count_type = "general filtered", .before = "count")
a2 <- count(btny_bind) %>% 
  rename(count = n) %>% 
  add_column(count_type = "unfiltered", .before = "count")
a3 <- count(
  btny_unique %>% filter(!is.na(`Type Citations (Scientific Name : Type Status : Verification Degree : Citation)`))) %>% 
  rename(count = n) %>% 
  add_column(count_type = "type specimens", .before = "count")

rbind(a1, a2, a3) %>% 
  ggplot(aes(x=count_type, y=count)) + 
  geom_bar(stat="identity", position="identity") +
  scale_x_discrete(limits = c("unfiltered", "general filtered", "type specimens"),
                   labels = c("unfiltered", "general filtered", "type specimens")) +
  theme_light()
```
